...
/// [`require_binary_rw_cursor`][require_binary_rw_cursor].
///
/// [require_binary_rw_cursor]: self::require_binary_rw_cursor
fn require_binary_rw_cursor_internal(
    attr: TokenStream,
    item: TokenStream,
...
pub(crate) fn require_binary_rw_cursor(attr: TokenStream, item: TokenStream) -> TokenStream {
    match require_binary_rw_cursor_internal(attr, item) {
        Ok(tokens) => tokens,
...
mod tests {
    ...
    /// Tests the `require_binary_rw_cursor_internal` function.
    #[test]
    fn require_binary_rw_cursor_internal_test() {
        let test_output: Item = parse2(
            require_binary_rw_cursor_internal(
                parse_quote! { C, crate },
        ...
        require_binary_rw_cursor_internal(parse_quote! { C }, parse_quote! { const X: u32 = 0; })
            .unwrap_err();
    }
